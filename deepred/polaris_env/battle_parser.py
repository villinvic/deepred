...
from deepred.polaris_env.pokemon_red.enums import BattleKind, RamLocation
...
def parse_battle_state(gamestate: GameState) -> BattleState:
    ...
    # print("".join([
    #     REVERSED_CHARMAP.get(c, "?") for c in
    #     read(slice(RamLocation.TILE_MAP_BASE + 250, RamLocation.TILE_MAP_BASE + 260, 1))
    # ]))

    # ?PIDGEY?????<NULL>???????????<LV>3???????????????<ID>’<BOLD_C><BOLD_L><BOLD_L><BOLD_L><BOLD_L><BOLD_L><BOLD_L><BOLD_M>??????????№ぁぁぁぁぁぁぁぁ?????????????????????????????#<PC>?????????????????<NEXT><CONT><TM>?????????????????<LINE><……><TRAINER>??CHARMELEON┌
    ...
    if gamestate.battle_kind == BattleKind.SAFARI:
        ...
        return BattleState.NICKNAME
    elif gamestate.open_menu and read(RamLocation.TILE_MAP_BASE + 12 * 20) != CHARMAP["┌"]:
        return BattleState.ACTIONABLE
